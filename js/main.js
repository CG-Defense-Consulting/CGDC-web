...
/* ============= Process Section Text Animation ============= */
(function() {
  const section = document.getElementById('company-intro');
  if (!section) return;

  const heading = section.querySelector('.company-intro-heading');
  const text = section.querySelector('.company-intro-text');
  ...
  // Prepare Heading: Split into chars for vertical slide animation
  if (heading) {
    // Helper to recursively process nodes and wrap characters
    const wrapChars = (node) => {
      if (node.nodeType === 3) { // Text node
        const content = node.nodeValue;
        if (content.trim().length === 0) return document.createTextNode(content);
        ...
        for (let char of content) {
          ...
          charSpan.className = 'char-slide';
          charSpan.textContent = char === ' ' ? '\u00A0' : char; // Use non-breaking space for layout
          
          clipSpan.appendChild(charSpan);
          fragment.appendChild(clipSpan);
        }
        return fragment;
      } else if (node.nodeType === 1 && node.tagName !== 'BR') { // Element node (recurse)
        // Clone the element to avoid modifying the live tree while traversing
        const clone = node.cloneNode(false);
        const childNodes = Array.from(node.childNodes);
        childNodes.forEach(child => {
          const processedChild = wrapChars(child);
          if (processedChild) {
        ...
        return node.cloneNode(true); // Keep BRs and other nodes as is
      }
    };
    ...
    childNodes.forEach(node => {
       const processed = wrapChars(node);
       if (processed) {
    ...
    heading.classList.add('process-heading-ready');
  }

  // Prepare Subtext
  if (text) {
    text.classList.add('process-text-ready');
  }
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        section.classList.add('animate-process');
        
        // Calculate and set delays for character stagger
        const charSlides = section.querySelectorAll('.char-slide');
        charSlides.forEach((char, index) => {
           // Stagger delay: 15ms per char
           // Add random variance for more natural feel or kept linear? 
           // Linear 20ms is good for text.
...
/* ============= Process Section Expansion Interaction ============= */
(function() {
  // Interaction removed as the section is now permanently expanded in a different location
  const content = document.getElementById('process-expandable-content');
  const inner = document.querySelector('.process-expanded-inner');
  
  if (content && !content.classList.contains('expanded')) {
    // Ensure it's visible if logic missed it in CSS (fallback)
